...
class PriceMachine:
    ...
    def export_search_results_to_html(self, found_items, filename='search_output.html'):
        if found_items.empty:
            print("Товар не найден.")
        else:
            html_table = found_items.to_html(index=False)
            with open(filename, 'w', encoding='utf-8') as f:
                f.write(html_table)
            print(f"Результаты поиска успешно сохранены в файл: {filename}")

    def find_text(self, search_term):
        search_term = search_term.lower()  # Преобразование поискового запроса в нижний регистр
        found_rows = []
        for index, row in self.main_df.iterrows():
            if search_term in row['наименование'].lower().split(' '):  # Сравнение в нижнем регистре
                found_rows.append(row)
        found_df = pd.DataFrame(found_rows)
        if not found_df.empty:
            print("Найденные товары:")
            print(found_df)
        self.export_search_results_to_html(found_df)
...
if __name__ == "__main__":
    ...
    while True:
        ...
        if search_term.lower() == "exit":
            ...
            price_machine.find_text(search_term)